...
class BaseDB(object):
	...
	def check_connection(self, db_name, db_user, db_password):
		...
		output = self.execute_query(self.get_db_server_status_query()).decode('utf-8')
		print(output)
	...
	def check_table(self, db_name, db_user, db_password, TABLE_NAME):
		...
		output = self.execute_query(self.get_check_table_query(TABLE_NAME)).decode('utf-8')
	...
	def hasDBnJavaPatchesEntries(self, db_name, db_user, db_password, version):
		output = self.execute_query(self.get_version_query(version,'Y')).decode('utf-8')
	...
	def import_core_db_schema(self, db_name, db_user, db_password, file_name,first_table,last_table):
		...
		if os.path.isfile(file_name):
			...
			if output.strip(version + " |"):
				...
				executePatch=False
				output = self.execute_query(self.get_version_query(version,'N')).decode('utf-8')
				if output.strip(version + " |"):
					#check whether process is in progress from different host if yes then wait until it finishes.
					output = self.execute_query(self.get_version_notupdatedby_query(version,'N',client_host)).decode('utf-8')
					...
					output = self.execute_query(self.get_unstale_patch_query(version,'N',client_host,stalePatchEntryHoldTimeInMinutes)).decode('utf-8')
					while(output.strip(version + " |")):
					...
					output = self.execute_query(self.get_stale_patch_query(version,'N',client_host,stalePatchEntryHoldTimeInMinutes)).decode('utf-8')
					if(output.strip(version + " |")):
	...
	def hasPendingPatches(self, db_name, db_user, db_password, version):
		output = self.execute_query(self.get_patch_status_in_current_version_query(version,ranger_version,'Y')).decode('utf-8')
		if output.strip(version + " |"):
			return False
		else:
			return True
